  ...
  getNewSetPiecePosition(pieceMap) {
    ...
    for (var i = xArr - 4; i < xArr; i++) {
      ...
      for (var j = 0; j < 4; j++) {
        ...
        var valueMap = convertValuesForSetPiece(
          pieceMap[contadorPieceY][contadorPieceX]
        );
    ...
    for (var i = 0; i < 4; i++) {
      for (var j = 0; j < 4; j++) {
        if (possiblePiece[i][j] == 1) {
          ...
          if (j + xArr > 9)
            return this.drawSetPieceTurningOnSideLimit(
              "rightLimit",
              possiblePiece
            );
          else if (j + xArr < 0)
    ...
    for (var i = 0; i < 4; i++) {
      for (var j = 0; j < 4; j++) {
        if (
          this.getMapPosition(
            ps.yArr + i,
            ps.xArr + j + (side == "right" ? 1 : -1)
          ) == 2 &&
          this.getMapPosition(
            ps.yArr + i,
            ps.xArr + j + (side == "right" ? 2 : -2)
          ) == 3
        ) {
          return true;
        }
    ...
    for (var i = 0; i < this.xArrayLength; i++) {
      for (var j = 0; j < this.yArrayLength; j++) {
        if (this.tetrisMap[j][i] == 2) {
          ...
          var square = this.scene.add
            .image(x, y, ps.pieceColorHash)
            .setOrigin(0, 0);
